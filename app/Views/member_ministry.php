...
.rmMinistry{font-size:14px; color:red; cursor: pointer;}
.rmMinistry:hover{ text-decoration:underline;}

#returnMinistries li:hover{ text-decoration:underline;cursor: pointer;}
...
<?= $header; ?>
	...
	<div class="oceanwp-mobile-menu-icon clr mobile-right">
		...
		<h1>My serving position(s) <?= (isset($disName)?' ('.$disName.')':'');?></h1>
		
		
		<?php if(!$ministry){
			echo '<p class="noLog">N/A</p>';
		}
		?>
		...
		<ul id="MyMinistries">
			...
			<?php 
				foreach($ministry as $item){
					...
					echo '<li>'.ucwords($label).' <span data-bid="'.$bid.'" data-ministryid="'.$item['id'].'" class="rmMinistry">x Remove</span></li>';
				}
			?>
...
<form>
		<input type="text" id="addMinistry" placeholder="Enter ministry here" />
...
$('#addMinistry').bind('input propertychange',  function() {
	doSearch();
});
...
function doSearch() {
	...
	var query_str = $('#addMinistry').val().trim();	
...
$( document ).on( "click", ".rmMinistry", function() {	
	...
	$('.rmMinistry').each(function(index) {
		arr.push( $(this).data('ministryid'));
	});	
	...
	$('#resultsMsg').html('Loading...');
		...
		timer = setTimeout(function() {
			...
			ajaxer = $.ajax({
				...
				method: "POST",
				url: memberUrl,
				data: {action:'removeMinistry',bid:bid,ministry:arr},   
	...
	$('#resultsMsg').html('Loading...');
		...
		timer = setTimeout(function() {
			...
			ajaxer = $.ajax({
				...
				method: "POST",
				url: memberUrl,
				data: {action:'addMinistry',ministryId:ministryId,ministryVal:ministryVal},   
