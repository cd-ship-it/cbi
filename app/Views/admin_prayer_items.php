		...
		<p>
			...
			<?php 
				...
				foreach($admins as $pastor){
					...
					if($prayer_id){
						$is_selected =  ($prayer_id == $pastor['bid'] ?'selected':'') ;					
					}else{
						$is_selected =  false ;
					}
...
需要協助翻譯的牧者: <br />
			...
			<?php 
				...
				foreach($admins as $pastor){	
					...
					echo '<span> <input name="prayerNeedHelp[]"  id="p'.$pastor['bid'].'" type="checkbox" '.(in_array($pastor['bid'],$prayerNeedHelp)?'checked':'').' value="'.$pastor['bid'].'"> <label for="p'.$pastor['bid'].'">'.$pastor['name'].'</label></span>';
...
翻譯員:  <input value="<?= (implode(', ',$prayerProvideHelp)); ?>" name="prayerProvideHelp" id="prayerProvideHelp" class="" />
	...
	<?php /*
 			...
 			<select class="prayerProvideHelp" name="prayerProvideHelp" id="prayerProvideHelp">
			<option value="">---</option>
			...
			<?php 
				...
				foreach($admins as $pastor){	
					...
					echo '<option  '.($prayerProvideHelp==$pastor['bid']?'selected':'').'  value="'.$pastor['bid'].'">'.$pastor['name'].'</option>';
				}
