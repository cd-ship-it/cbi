...
<a style=" margin: 0 10px 20px 10px; " href="javascript:void(0);" class="selected tditem ">+New Group Tag</a>
...
foreach($tags as $key => $item){
	...
	echo '<tr data-tagid="'.$item['id'].'" class="'.$class.'">';
...
#wrapNode{background: #fff; position: fixed; width: 330px; left: 50%; margin-left: -150px; margin-top: 15%; padding: 25px; box-shadow: 0 0 20px 2px #000; z-index:9999;}
...
$( document ).on( "click", ".tditem", function() {
	...
	if($(this).hasClass('selected')){		
		...
		sbNode.innerHTML =   tagid ? 'Update' : 'Submit';
		...
		sbNode.addEventListener('click', function(){
			...
			var data = {
				...
				ajaxer = $.ajax({
					dataType:'html',
					method: "POST",
					url: '<?php echo base_url("xAdmin/group/tags"); ?>',
					data: data,      
