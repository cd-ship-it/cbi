...
<?php
	...
	foreach($curriculums as $curriculum){
		...
		echo '<div class="row my-3"><div class="col-md-12 stretch-card"><div class="card shadow mb-4"><div class="card-body"><div class="curriculumDiv">';
		
		$dateS = date("m/d/Y",$curriculum['start']);
		$dateE = date("m/d/Y",$curriculum['end']);
		
		if(!$curriculum['is_active']) echo ' <span style="color: #ffffff;background: #ffafaf;padding: 0px 5px;margin-bottom: 10px;display: inline-block;">Cancelled</span>';
		
		echo '<div class="curriculumInfo"><h3><a data-lesson="'.$curriculum['sessions'].'" id="title'.$curriculum['id'].'" href="'.(!$isElearningClass?base_url('xAdmin/edit_curriculum/'.$curriculum['id']):'javascript: void(0);').'">'.$curriculum['title'].' | Edit Class</a><span>(Participants: '.count($students).')</span></h3>';
...
.curriculumInfo{  position: relative;  }
...
$( document ).on( "click", ".messageSendcancel", function() {
	...
	selectInverse.innerHTML = 'Invert Selection';	
...
function displayMsgLogs($cid,$logs){
	...
	foreach($logs as $log){
		...
		<h4>'.ucwords($log['sender']).' sent a broadcast message on '.date("m/d/Y g:i a",$log['date']).'</h4>
		<h4>Message Content:</h4><div class="msgcon">'.$message.'</div>
		<h4>Recipients:</h4><div class="msgto">'.implode(', ',$send_to_arr[0]).'</div>	
		</div>';
	}
	
	if($logsHtml==''){
		$logsHtml = '<div class="nologs">N/A</div>';
	}
	
	echo '
	<div class="classMsgLogs" id="msgLogs'.$cid.'">
	<h3>Logs</h3>
	<div id="logsWrap'.$cid.'">'.$logsHtml.'</div></div>';	
};
...
function displayFunction($cid,$students,$cTitle){
	...
	<div class="classFunction" id="cf'.$cid.'">
			...
			<div class="stlists stlists_'.$cid.'"><h4>Send to:</h4><button class="selectAll" data-cid="'.$cid.'">Select All</button> <button class="selectInverse" data-cid="'.$cid.'">Invert Selection</button><div class="stuhtml">'.$stuhtml.'</div></div>
