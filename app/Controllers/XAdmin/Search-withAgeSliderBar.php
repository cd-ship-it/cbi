...
class Search extends BaseController
{
	...
	public function index($mode=0,$filter='')
	{
		...
		if($action == 'search'){  				
				...
				$condition = [];
				if(preg_match_all('#[a-z]+#i',$_POST['query'],$keys)){
					foreach($keys[0] as $k){
				...
				if($_POST['ageSlider'] && $ageSlider_arr = explode(',',$_POST['ageSlider']) ){
					
					if($ageSlider_arr[0]>0||$ageSlider_arr[1]<99){
						$condition[] =  "TIMESTAMPDIFF(YEAR,FROM_UNIXTIME(a.birthDate-8*3600),CURDATE()) BETWEEN {$ageSlider_arr[0]} AND {$ageSlider_arr[1]}";
				...
				// if($_POST['over18']){
					// $condition[] =  "TIMESTAMPDIFF(YEAR,FROM_UNIXTIME(a.birthDate-8*3600),CURDATE()) >= 18";
				// }
			...
			$r = $modelProfilesModel->update($mid, array($cc => $value));
		...
		if($filter){
			...
			$data['inputAge1'] = preg_match('#BETWEEN (\d+) AND (\d+)#',$conditionStr,$match)?$match[1]:'';
			$data['inputAge2'] = preg_match('#BETWEEN (\d+) AND (\d+)#',$conditionStr,$match)?$match[2]:'';
...
private function mode1html($links,$results){ 
	...
	if($results){
		...
		$columns = ['Name','Email',	'Gender',	'Marital',	'Birth Date', 'Age','Occupation','Date of your baptism','Site','Membership Date'];
		...
		foreach($results as $item){
			...
			$html .=  '<td class=""><a class="profile" target="_blank" href="'.base_url('xAdmin/baptist/'.$id).'">'.$item['fName'].' '.$item['mName'].' '.$item['lName'].'</a></td>';
...
private function mode0html($links,$results,$ingClassesIds,$part=1){ 
	...
	if($results){
		...
		foreach( $results as $item) {
			...
			$html .=  '<td><a class="profile" target="_blank" href="'.base_url('xAdmin/baptist/'.$item['id']).'">'.$item['name'].'</a></td><td>';
